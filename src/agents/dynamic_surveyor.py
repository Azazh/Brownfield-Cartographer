...
class DynamicSurveyor:
    def __init__(self, knowledge_graph: KnowledgeGraph, trace_logger=None):
        self.kg = knowledge_graph
        self.router = LanguageRouter()
        self.trace_logger = trace_logger


    def analyze_repo(self, repo_path: str, changed_files=None, added_files=None, deleted_files=None) -> dict:
        """
        Analyze the repository, build the module graph (multi-language), compute analytics,
        and return a structured report. Supports incremental update via changed_files, added_files, deleted_files.
        """
        if changed_files or added_files or deleted_files:
            logger.info(f"[Surveyor] Incremental update: changed={changed_files}, added={added_files}, deleted={deleted_files}")
            # Remove deleted files from the knowledge graph
            if deleted_files:
            ...
            # Continue to analytics and report as in full run
            ...
            return self.analyze_repo_full(repo_path)


    def analyze_repo_full(self, repo_path: str) -> dict:
        """
        Analyze the repository, build the module graph (multi-language), compute analytics,
