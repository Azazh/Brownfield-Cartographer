...
def run_analysis(repo_path: str, output_dir: str = '.cartography', sql_dialect: str = 'duckdb', run_mode: str = 'auto'):
    """
    ...
    # --- Incremental/full/auto update logic ---
    ...
    # Determine run mode
    if run_mode == 'full':
        logger.info("Forcing full analysis (all files).")
        changed_files = added_files = deleted_files = []
    elif run_mode == 'incremental':
        logger.info("Forcing incremental analysis (if possible).")
        if not (last_commit and (changed_files or added_files or deleted_files)):
            logger.warning("No previous commit or no changes detected; falling back to full analysis.")
            changed_files = added_files = deleted_files = []
    else:
        # auto: use incremental if possible, else full
    ...
    hydrologist_report = None
    try:
        hydrologist_report = hydrologist.analyze_repo(repo_path, changed_files=changed_files, added_files=added_files, deleted_files=deleted_files)
        if hasattr(hydrologist_report, 'to_json_serializable'):
            hydrologist_report = hydrologist_report.to_json_serializable()
    ...
    try:
        semantic_report = semanticist.analyze_repo(repo_path, surveyor_report=report, hydrologist_report=hydrologist_report, changed_files=changed_files, added_files=added_files, deleted_files=deleted_files)
        ...
        with open(semantic_report_path, 'w', encoding='utf-8') as f:
            json.dump(semantic_report if semantic_report is not None else {}, f, indent=2)
    ...
    try:
        ...
        logger.warning(f"Archivist phase failed: {e}")
